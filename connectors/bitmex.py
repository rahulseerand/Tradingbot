...
class BitmexClient:
    ...
    def cancel_order(self, order_id: str) -> OrderStatus:
        ...
        if 'error' in order_status:
            logger.error("Error response from API: %s", order_status['error'])
            return OrderStatus({'ordStatus': 'Error', 'error': order_status['error']}, "bitmex")
        elif isinstance(order_status, list) and order_status:
            # Assuming the response is a list of order statuses
            order_status = order_status[0]
            if 'ordStatus' not in order_status:
                logger.error("ordStatus not in response: %s", order_status)
                return OrderStatus({'ordStatus': 'Error', 'error': 'Unknown Error'}, "bitmex")
            return OrderStatus(order_status, "bitmex")
        else:
            logger.error("Unexpected response format: %s", order_status)
            return OrderStatus({'ordStatus': 'Error', 'error': 'Unknown Format'}, "bitmex")
